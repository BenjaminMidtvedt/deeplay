...
def parse_args(args):
    ...
    while i < len(args):
        ...
        if arg.startswith('--exclude='):
            ...
            if i + 1 < len(args):
                ...
                print("Error: --exclude requires a comma-separated list, e.g. --exclude=mod1,mod2")
                sys.exit(1)
            ...
            if i + 1 < len(args):
                ...
                print("Error: --output-dir requires a directory name, e.g. --output-dir=docs")
                sys.exit(1)
        else:
    ...
    if len(positional) < 1:
        print("Usage: python generate_docs.py <library_name> [--force|-f] [--exclude|-e=mod1,mod2,...] [--output-dir|-o <dir>]")
        sys.exit(1)
...
def main():
    ...
    if force or not os.path.exists(doc_filename):
        with open(doc_filename, "w", encoding="utf-8") as doc_file:
            ...
            doc_file.write("=============\n\n")
            doc_file.write(f"Here, you will find the documentation for {library_name}.\n")
            doc_file.write("The documentation is organized into the following sections:\n\n")
    ...
    for mod in submodules:
        ...
        if force or not os.path.exists(rst_filename):
            ...
            print(f"{rst_filename} already exists. Skipping (no --force flag provided).")
